...
export default function ResponsePlan({
  ...
  const handleMemoryFileSelection = useCallback(async (event) => {
    ...
    try {
      ...
      setDriveStatus(
        `${prepared.length} image(s) ready for Google Drive upload.`
      );
    } catch (error) {
    ...
    try {
      ...
      for (let index = 0; index < selectedMemoryFiles.length; index += 1) {
        ...
        const response = await fetch(
          `${API_BASE_URL}/api/v1/history/memories`,
          {
            method: "POST",
            headers: {
            ...
            body: JSON.stringify({
              title: `Memory ${String(index + 1).padStart(
                2,
                "0"
              )} · ${folderName}`,
  ...
  const guideVisaQuickFacts = useMemo(() => {
    const visa = guideCompanion?.visaQuickFacts;
    if (!visa || typeof visa !== "object") return null;

    return {
      travelerCitizenship: String(visa.travelerCitizenship || "").trim(),
      destination: String(visa.destination || "").trim(),
      entryRequirement: String(visa.entryRequirement || "").trim(),
      maxVisaFreeStay: String(visa.maxVisaFreeStay || "").trim(),
      typicalProcessingTime: String(visa.typicalProcessingTime || "").trim(),
      estimatedFee: String(visa.estimatedFee || "").trim(),
      requiredDocuments: toGuideList(visa.requiredDocuments, []),
      extraNotes: toGuideList(visa.extraNotes, []),
      officialSourceHint: String(visa.officialSourceHint || "").trim(),
    };
  }, [guideCompanion]);
          ...
          <div className="space-y-6">
            ...
            {active === "guide" ? (
              <GuideTab
                ...
                guideVisaQuickFacts={guideVisaQuickFacts}
              />
            ) : null}

            {active === "wallet" ? (
              <WalletTab budgetItems={budgetItems} />
            ) : null}
