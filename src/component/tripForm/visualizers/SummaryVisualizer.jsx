...
export default function SummaryVisualizer({ data }) {
  const hasOrigin = !!(
    data.context?.homeCountry || data.context?.departureCity
  );
  const hasDest =
    ...
    <div className="flex h-full w-full flex-col overflow-hidden rounded-2xl border border-[#dfd3bf] bg-white shadow-[0_8px_26px_rgba(0,0,0,0.06)]">
      <div className="border-b border-[#e4d8c4] bg-[#f9f2e5] p-6">
        <div className="flex items-center justify-between mb-2">
          ...
          <span
            className={`rounded px-2 py-1 text-[10px] font-bold uppercase tracking-wide border ${badge}`}
          >
            {data.tripStatus === "booked" ? "Confirmed Trip" : "Planning Phase"}
          </span>
        </div>

        <h2 className="font-display text-[34px] leading-[1] text-[var(--ink)]">
          Trip Snapshot
        </h2>
        <div className="mt-2 text-[13px] text-[#556972]">
          We’ll use your answers to generate itinerary, logistics, and budget
          strategy.
        </div>
      </div>
      ...
      <div className="flex-1 p-6 overflow-y-auto space-y-6">
        ...
        <Row
          ...
          text={
            hasOrigin
              ? `${data.context?.departureCity || "City"}, ${
                  data.context?.homeCountry || "Country"
                }`
              : "Not set yet"
          }
          sub={
            data.context?.currency ? `Currency: ${data.context.currency}` : ""
          }
        />
              ...
              <div className="mt-1 flex flex-wrap gap-2">
                {[
                  ...
                  .map((x) => (
                    <span
                      key={x}
                      className="rounded-md border border-[#dacdb9] bg-[#fff8eb] px-2 py-1 text-xs font-medium text-[#3f5660]"
                    >
                      {x}
                    </span>
                  ))}
        ...
        <Row
          ...
          sub={
            data.tripStatus === "planning"
              ? `Duration: ~${data.dates?.durationDays || 7} days`
              : ""
          }
        />
        ...
        <Row
          ...
          text={
            hasBudget
              ? `$${Number(data.budget.usdBudget).toLocaleString()} USD`
              : "Not set"
          }
    ...
    <div className="flex items-start gap-4">
      <div
        className={`mt-1 rounded-lg p-2 ${
          ok ? "bg-[#e8f7f4] text-[#0c5f5c]" : "bg-slate-100 text-slate-400"
        }`}
      >
        {icon}
      </div>
      <div className="flex-1 min-w-0">
        <div className="flex items-center justify-between">
          <div className="text-xs font-semibold uppercase tracking-wide text-[#5d7078]">
            {title}
          </div>
          {ok ? (
        ...
        {custom ? (
          ...
          <div
            className={`mt-1 text-sm font-medium ${
              ok ? "text-[var(--ink)]" : "text-slate-400 italic"
            }`}
          >
            {text}
          </div>
